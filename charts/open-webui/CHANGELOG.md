...
### Changed
- Updated chart appVersion to v0.11.4.

## [v16.5.0]

### Changed
- Bumped subchart dependency versions: `ollama` 1.70.0 → 1.79.0, `pipelines` 0.12.0 → 0.12.2, `terminals` 0.5.0 → 0.7.0.

## [v16.4.0]

### Added
- `route.timeouts` exposes the HTTPRoute `timeouts` field (`request`/`backendRequest`). (#421)
...
### Added
- `WEBUI_URL` is now automatically derived from `route.hostnames` when using a Gateway API route (`route.enabled: true`), the same way it already is from `ingress.host`. It no longer needs to be set manually via `extraEnvVars` in this case.
...
### Changed
- Updated chart appVersion to v0.11.0.
- Updated Ollama subchart version to v1.70.0.

## [v15.2.0]

### Changed
- Updated chart appVersion to v0.10.2.
- Updated Ollama subchart version to v1.65.0.

## [v15.1.0]

### Changed
Updated chart appVersion to v0.10.1.

## [v15.0.0]

### Changed
Updated chart appVersion to v0.10.0.

## [v14.11.0]

### Added

- Add `service.appProtocol` value to set the [`appProtocol`](https://kubernetes.io/docs/concepts/services-networking/service/#application-protocol) on the Service's `http` port (e.g. `kubernetes.io/ws` for WebSocket), so load balancers and ingress controllers that key off `appProtocol` can route Open WebUI's WebSocket traffic correctly. Defaults to `""` (unset), preserving existing behavior.
...
- Bump `terminals` subchart dependency to `>=0.4.0`, which fixes Terminal PVC provisioning on managed Kubernetes (AWS EKS, GKE, AKS) and exposes new `terminalDefaults.persistence.{enabled,size,storageClass}` values ([#392](https://github.com/open-webui/helm-charts/issues/392))
...
### Changed
Updated chart appVersion to v0.9.5.

## [v14.4.0]

### Changed
Updated chart appVersion to v0.9.4.

## [v14.3.0]

### Changed
- Updated chart appVersion to v0.9.3.
- Updated Ollama subchart to v1.56.0.
- Updated minimum Terminals chart version to v0.3.0.

## [v14.2.0]

### Changed
Updated chart appVersion to v0.9.2, updated Ollama subchart version.

## [v14.1.0]

### Changed
Updated chart appVersion to v0.9.1.

## [v14.0.0]

### Changed
Updated chart appVersion to v0.9.0.
